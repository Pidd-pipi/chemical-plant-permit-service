...
import (
	...
	"example.com/chemical-plant-permit-service/validation"
	"net/http"
	"strings"
)
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodGet && r.URL.Path == "/api/v1/permits" {
		items := domain.SortPermitsByRisk(h.Store.List())
		write(w, 200, map[string]any{"items": items})
		return
	}
	if r.Method == http.MethodPost && strings.HasPrefix(r.URL.Path, "/api/v1/permits/") {
		h.update(w, r)
		return
	}
...
func (h *Handler) update(w http.ResponseWriter, r *http.Request) {
	...
	if len(p) != 5 || p[0] != "api" || p[1] != "v1" || p[2] != "permits" || p[4] != "status" || p[3] == "" {
		http.Error(w, "route not found", 404)
		return
	}
	...
	if errors.Is(e, store.ErrNotFound) {
		http.Error(w, "permit not found", 404)
		return
	}
	write(w, 200, item)
}
