...
// cleanupExpired 清理已关闭的许可记录并返回清理数量。
func (s *OpsService) cleanupExpired(now time.Time) int {
	...
	removed := 0
	for _, item := range items {
		if item.Status == OpsStatusClosed {
			if err := s.store.Delete(context.Background(), item.ID); err == nil {
				removed++
			}
		}
	}
